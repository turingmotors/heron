...
def generate_response(question, image, model, processor, device):
    """
    Generates a response for a given question and image, with '##human: ' prefix removed.
    """
    ...
    response = processor.tokenizer.batch_decode(out, skip_special_tokens=True)[0]
...
def process_questions(img_root, questions, model, processor, device, verbose):
    ...
    for q in tqdm(questions):
        image = Image.open(os.path.join(img_root, f"{q['image']}"))
        question = q["jp"]
...
def upload_results(img_root, results, name):
    ...
    for r in results:
        image = wandb.Image(
            Image.open(os.path.join(img_root, f"{r['image']}")), caption=r["answer"]
        )
        table.add_data(r["question_id"], name, image, r["jp"], r["answer"])
